/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 *  .Net Core Plugin Manager is distributed under the GNU General Public License version 3 and  
 *  is also available under alternative licenses negotiated directly with Simon Carter.  
 *  If you obtained Service Manager under the GPL, then the GPL applies to all loadable 
 *  Service Manager modules used on your system as well. The GPL (version 3) is 
 *  available at https://opensource.org/licenses/GPL-3.0
 *
 *  This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY,
 *  without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
 *  See the GNU General Public License for more details.
 *
 *  The Original Code was created by Simon Carter (s1cart3r@gmail.com)
 *
 *  Copyright (c) 2018 - 2023 Simon Carter.  All Rights Reserved.
 *
 *  Product:  SimpleDB
 *  
 *  File: SimpleDBOperations.cs
 *
 *  Purpose:  SimpleDBOperations for SimpleDB
 *
 *  Date        Name                Reason
 *  23/05/2022  Simon Carter        Initially Created
 *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
using System.Reflection;
using System.Runtime.CompilerServices;
using System.Text;
using System.Text.Json;

using PluginManager.Abstractions;

using Shared.Classes;

using SharedPluginFeatures;

using SimpleDB.Internal.Tables;

namespace SimpleDB.Internal
{
    /// <summary>
    /// Internal structure for file is:
    /// 
    /// ushort      Internal data version					0
    /// byte[2]     Header									2
    /// long        Primary Sequence						4
    /// long        Secondary Sequence						12
    /// ushort		File Version							20
    /// ushort		Reserved								22
    /// int         Reserved for future use					24
    /// int         Reserved for future use					28
    /// int         Page size								32
    /// byte        Compression								36
    /// int         Record Count							37
    /// int         Length of data before compression		41
    /// int         Length of data stored on disk			45
    /// Data		version specific						53
    /// </summary>
    /// <typeparam name="T"></typeparam>
    internal sealed class SimpleDBOperations<T> : ISimpleDBOperations<T>, ISimpleDBTable, IWalUndoTarget, ICheckpointableTable, ICompactableTable<T>, IMaintainableTable
        where T : TableRowDefinition
    {
        #region Private Classes

        private sealed class ForeignKeyRelation
        {
            public ForeignKeyRelation(string name, ForeignKeyAttributes foreignKeyAttributes)
            {
                if (String.IsNullOrEmpty(name))
                    throw new ArgumentNullException(nameof(name));

                Name = name;
                Attributes = foreignKeyAttributes;
            }

            public string Name { get; }

            public ForeignKeyAttributes Attributes { get; }
        }

        #endregion Private Classes

        #region Private Members

        private const int DefaultSequenceId = -1;
        private const string TimingsSelectAll = "TimingsSelectAll";
        private const string TimingsSelectId = "TimingsSelectId";
        private const string TimingsSelectPredicate = "TimingsSelectPredicate";
        private const string TimingsInsertList = "TimingsInsertList";
        private const string TimingsInsert = "TimingsInsert";
        private const string TimingsDeleteList = "TimingsDeleteList";
        private const string TimingsDelete = "TimingsDelete";
        private const string TimingsTruncate = "TimingsTruncate";
        private const string TimingsUpdateList = "TimingsUpdateList";
        private const string TimingsUpdate = "TimingsUpdate";
        private const string TimingsInsertOrUpdate = "TimingsInsertOrUpdate";
        private const string TimingsForceWrite = "TimingsForceWrite";
        private const string TimingsRecovery = "TimingsRecovery";

        private readonly IVersionedReadWriteFactory _readWriteFactory;
        private readonly IWalRecorder _walRecorder;
        private readonly IWalTableAccessor _walTableAccessor;
        private readonly IWatermarkAccessor _watermarkAccessor;
        private readonly string _tableName;
        private readonly bool _tableCreated;
        private FileStream _fileStream;
        private bool _closedForMaintenance;
        private ushort _internalDataVersion;
        private ushort _internalWriteVersion;
        private bool _disposed;
        private int _recordCount = 0;
        private int _dataLength = 0;
        private int _physicalDataLength = 0;
        private long _inMemoryCacheSizeBytes = 0;
        private byte _compactPercent = 0;
        private int _pageCount;
        private long _primarySequence = DefaultSequenceId;
        private long _SecondarySequence = DefaultSequenceId;
        private readonly object _lockObject = new();
        private readonly TableAttribute _tableAttributes;
        private readonly Dictionary<string, ForeignKeyRelation> _foreignKeys;
        private readonly ISimpleDBManager _simleDBManager;
        private bool _hasInitialized;
        private readonly IForeignKeyManager _foreignKeyManager;
        private readonly ITransactionManager _transactionManager;
        private BatchUpdateDictionary<string, IIndexManager> _indexes;
        private readonly Dictionary<TriggerType, List<ITableTriggers<T>>> _triggersMap;
        private PageSize _pageSize;
        private List<T> _allRecords = null;
        private readonly bool _isMemoryCaching;
        private readonly Dictionary<string, Timings> _ReadWriteTimes = new()
            {
                { TimingsSelectAll, new() },
                { TimingsSelectId, new() },
                { TimingsSelectPredicate, new() },
                { TimingsInsertList, new() },
                { TimingsInsert, new() },
                { TimingsDeleteList, new() },
                { TimingsDelete, new() },
                { TimingsTruncate, new() },
                { TimingsUpdateList, new() },
                { TimingsUpdate, new() },
                { TimingsInsertOrUpdate, new() },
                { TimingsForceWrite, new() },
                { TimingsRecovery, new() }
            };

        #endregion Private Members

        #region Constructors / Destructors

        public SimpleDBOperations(ISimpleDBManager readerWriterInitializer, IForeignKeyManager foreignKeyManager,
            ITransactionManager transactionManager, IWalTableAccessor walTableAccessor, IWatermarkAccessor watermarkAccessor)
        {
            _simleDBManager = readerWriterInitializer ?? throw new ArgumentNullException(nameof(readerWriterInitializer));
            _foreignKeyManager = foreignKeyManager ?? throw new ArgumentNullException(nameof(foreignKeyManager));
            _transactionManager = transactionManager ?? throw new ArgumentNullException(nameof(transactionManager));
            _walTableAccessor = walTableAccessor ?? throw new ArgumentNullException(nameof(walTableAccessor));
            _watermarkAccessor = watermarkAccessor ?? throw new ArgumentNullException(nameof(watermarkAccessor));

            _tableAttributes = GetTableAttributes() ?? throw new InvalidOperationException($"TableAttribute is missing from class {typeof(T).FullName}");
            _readWriteFactory = ResolveReadWriteFactory();
            _walRecorder = ResolveWalRecorder();

            // All tables are fully in-memory now that the WAL is the source of truth at runtime -
            // table files are only ever flushed via startup recovery or a checkpoint, never per-DML.
            _isMemoryCaching = true;

            _triggersMap = [];

            foreach (TriggerType triggerType in Enum.GetValues(typeof(TriggerType)))
                _triggersMap.Add(triggerType, []);

            _foreignKeys = GetForeignKeysForTable();
            _indexes = BuildIndexListForTable();

            (_tableCreated, _tableName) = ValidateTableName(_simleDBManager.Path, _tableAttributes.Domain, _tableAttributes.TableName, _tableAttributes.PageSize);
            _fileStream = File.Open(_tableName, FileMode.Open, FileAccess.ReadWrite, FileShare.Read);

            try
            {
                ValidateTableContents();
            }
            catch (InvalidDataException)
            {
                _fileStream.Dispose();
                _fileStream = null;
                GC.SuppressFinalize(this);

                throw;
            }

            _simleDBManager.RegisterTable(this);
            _foreignKeyManager.RegisterTable(this);

            PerformStartupRecovery();

            RebuildAllMissingIndexes();
        }

        ~SimpleDBOperations()
        {
            Dispose(false);
        }

        #endregion Constructors / Destructors

        #region ISimpleDBTable

        #region Properties

        public string TableName
        {
            get
            {
                if (_tableAttributes != null)
                    return _tableAttributes.TableName;

                return null;
            }
        }

        public TimeSpan SlidingMemoryTimeout => _tableAttributes.SlidingMemoryTimeout;

        public CachingStrategy CachingStrategy => _tableAttributes.CachingStrategy;

        public WriteStrategy WriteStrategy => _tableAttributes.WriteStrategy;

        public Dictionary<string, Timings> GetAllTimings
        {
            get
            {
                Dictionary<string, Timings> Result = [];

                foreach (KeyValuePair<string, Timings> item in _ReadWriteTimes)
                {
                    Result.Add(item.Key, item.Value.Clone());
                }

                return Result;
            }
        }

        /// <summary>
        /// Total logical size (uncompressed) of the data held by the table, in bytes
        /// </summary>
        public long LogicalDataSizeBytes
        {
            get
            {
                try
                {
                    return Volatile.Read(ref _dataLength);
                }
                catch
                {
                    return 0;
                }
            }
        }

        /// <summary>
        /// Total physical size (on-disk / stored) of the data held by the table, in bytes
        /// </summary>
        public long PhysicalDataSizeBytes
        {
            get
            {
                try
                {
                    return Volatile.Read(ref _physicalDataLength);
                }
                catch
                {
                    return 0;
                }
            }
        }

        /// <summary>
        /// Size in bytes of the records currently held in the in-memory cache (if any)
        /// </summary>
        public long InMemoryCacheSizeBytes
        {
            get
            {
                try
                {
                    return Volatile.Read(ref _inMemoryCacheSizeBytes);
                }
                catch
                {
                    return 0;
                }
            }
        }

        #endregion Properties

        #region Methods

        public void Initialize(IPluginClassesService pluginClassesService)
        {
            if (pluginClassesService == null)
                throw new ArgumentNullException(nameof(pluginClassesService));

            if (_hasInitialized)
                return;

            ITableDefaults<T> tableDefaults = pluginClassesService.GetPluginClasses<ITableDefaults<T>>()
                .FirstOrDefault();

            List<ITableTriggers<T>> triggers = pluginClassesService.GetPluginClasses<ITableTriggers<T>>();

            foreach (TriggerType triggerType in Enum.GetValues(typeof(TriggerType)))
                _triggersMap[triggerType].AddRange(triggers.Where(t => t.TriggerTypes.HasFlag(triggerType)).ToList());

            if (_tableCreated && tableDefaults != null)
            {
                if (_primarySequence == DefaultSequenceId)
                    _primarySequence = tableDefaults.PrimarySequence;

                if (_SecondarySequence == DefaultSequenceId)
                    _SecondarySequence = tableDefaults.SecondarySequence;

                if (tableDefaults.InitialData != null)
                {
                    for (ushort i = ++_internalDataVersion; i < ushort.MaxValue; i++)
                    {
                        List<T> initialData = tableDefaults.InitialData(i);

                        if (initialData == null || initialData.Count == 0)
                            break;

                        Insert(initialData);

                        _internalDataVersion = InternalUpdateVersion(i);
                    }
                }
            }

            _hasInitialized = true;
        }

        public bool IdExists(long id)
        {
            return _indexes[nameof(TableRowDefinition.Id)].Contains(id);
        }

        public bool IndexExists(string name, object value)
        {
            if (String.IsNullOrEmpty(name))
                throw new ArgumentNullException(nameof(name));

            if (!_indexes.TryGetValue(name, out IIndexManager indexManager))
                throw new ArgumentOutOfRangeException($"Index {name} does not exist");

            return indexManager.Contains(value);
        }

        public bool IdIsInUse(string propertyName, long value)
        {
            foreach (T record in Select())
            {
                long keyValue = Convert.ToInt64(record.GetType().GetProperty(propertyName).GetValue(record, null));

                if (value.Equals(keyValue))
                    return true;
            }

            return false;
        }

        #endregion Methods

        #region Events

        public event SimpleDbEvent OnAction;

        #endregion Events

        #endregion ISimpleDBTable

        #region ISimpleDBOperation<T>

        #region Properties

        public ushort FileVersion => _internalDataVersion;

        public int DataLength => _dataLength;

        public int RecordCount => _recordCount;

        public long PrimarySequence => _primarySequence;

        public long SecondarySequence => _SecondarySequence;

        public byte CompactPercent => _compactPercent;

        public int PageCount => _pageCount;

        public PageSize PageSize => _pageSize;

        public object TableLock => _lockObject;

        #endregion Properties

        public void ClearAllMemory()
        {
            //using (TimedLock timedLock = TimedLock.Lock(_lockObject, SimpleDBConfiguration.ClearMemoryLockTimeout))
            //{
            //    if (_allRecords != null)
            //        InternalSaveRecordsToDisk(_allRecords, true, null);

            //    _allRecords = null;
            //    Volatile.Write(ref _inMemoryCacheSizeBytes, 0);
            //}
        }

        public IReadOnlyList<T> Select()
        {
            using (StopWatchTimer timer = StopWatchTimer.Initialise(_ReadWriteTimes[TimingsSelectAll]))
            {
                if (_disposed)
                    throw new ObjectDisposedException(nameof(SimpleDBOperations<T>));

                return InternalReadAllRecords().AsReadOnly();
            }
        }

        public T Select(long id)
        {
            using (StopWatchTimer timer = StopWatchTimer.Initialise(_ReadWriteTimes[TimingsSelectId]))
            {
                if (_disposed)
                    throw new ObjectDisposedException(nameof(SimpleDBOperations<T>));

                return InternalReadAllRecords().Find(r => r.Id.Equals(id));
            }
        }

        public IReadOnlyList<T> Select(Func<T, bool> predicate)
        {
            using (StopWatchTimer timer = StopWatchTimer.Initialise(_ReadWriteTimes[TimingsSelectPredicate]))
            {
                if (_disposed)
                    throw new ObjectDisposedException(nameof(SimpleDBOperations<T>));

                if (predicate == null)
                    throw new ArgumentNullException(nameof(predicate));

                return InternalReadAllRecords().Where(predicate).ToList().AsReadOnly();
            }
        }

        public void Insert(List<T> records)
        {
            Insert(records, new InsertOptions());
        }

        public void Insert(List<T> records, InsertOptions insertOptions)
        {
            ITransaction transaction = _transactionManager.BeginTransaction();
            Insert(records, insertOptions, transaction);
            transaction.Commit();
        }

        public void Insert(T record)
        {
            Insert(record, new InsertOptions());
        }

        public void Insert(T record, InsertOptions insertOptions)
        {
            ITransaction transaction = _transactionManager.BeginTransaction();
            Insert(record, insertOptions, transaction);
            transaction.Commit();
        }

        public void Delete(List<T> records)
        {
            ITransaction transaction = _transactionManager.BeginTransaction();
            Delete(records, transaction);
            transaction.Commit();
        }

        public void Delete(T record)
        {
            ITransaction transaction = _transactionManager.BeginTransaction();
            Delete(record, transaction);
            transaction.Commit();
        }

        public void Truncate()
        {
            ITransaction transaction = _transactionManager.BeginTransaction();
            Truncate(transaction);
            transaction.Commit();
        }

        public void Update(List<T> records)
        {
            ITransaction transaction = _transactionManager.BeginTransaction();
            Update(records, transaction);
            transaction.Commit();
        }

        public void Update(T record)
        {
            ITransaction transaction = _transactionManager.BeginTransaction();
            Update(record, transaction);
            transaction.Commit();
        }

        public void InsertOrUpdate(T record)
        {
            ITransaction transaction = _transactionManager.BeginTransaction();
            InsertOrUpdate(record, transaction);
            transaction.Commit();
        }

        public void ForceWrite()
        {
            using (StopWatchTimer timer = StopWatchTimer.Initialise(_ReadWriteTimes[TimingsForceWrite]))
            {
                InternalCheckpoint();
            }
        }

        #region ICheckpointableTable

        public bool ParticipatesInWal => TableParticipatesInWal();

        public void AdvanceCheckpointWatermark(long lastFlushedSequence)
        {
            using (TimedLock timedLock = TimedLock.Lock(_lockObject))
            {
                _watermarkAccessor.SetLastFlushedSequence(TableName, lastFlushedSequence);
            }
        }

        #endregion ICheckpointableTable

        #region IMaintainableTable

        public void CloseForMaintenance()
        {
            using (TimedLock timedLock = TimedLock.Lock(_lockObject))
            {
                if (_closedForMaintenance)
                    return;

                if (_fileStream != null)
                {
                    InternalCheckpoint();
                    _fileStream.Flush(true);
                    _fileStream.Close();
                    _fileStream.Dispose();
                    _fileStream = null;
                }

                _allRecords = null;
                Volatile.Write(ref _inMemoryCacheSizeBytes, 0);
                _closedForMaintenance = true;
            }
        }

        public void ReopenAfterMaintenance()
        {
            using (TimedLock timedLock = TimedLock.Lock(_lockObject))
            {
                if (!_closedForMaintenance)
                    return;

                (_, string tableName) = ValidateTableName(_simleDBManager.Path, _tableAttributes.Domain, _tableAttributes.TableName, _tableAttributes.PageSize);
                _fileStream = File.Open(tableName, FileMode.Open, FileAccess.ReadWrite, FileShare.Read);

                ValidateTableContents();

                _indexes = BuildIndexListForTable();

                _closedForMaintenance = false;

                PerformStartupRecovery();

                RebuildAllMissingIndexes();
            }
        }

        #endregion IMaintainableTable

        #region ICompactableTable<T>

        public void Compact(Func<T, bool> keep)
        {
            if (keep == null)
                throw new ArgumentNullException(nameof(keep));

            using (TimedLock timedLock = TimedLock.Lock(_lockObject))
            {
                List<T> existingRecords = InternalReadAllRecords();
                List<T> survivors = [];
                List<T> discarded = [];

                foreach (T record in existingRecords)
                {
                    if (keep(record))
                        survivors.Add(record);
                    else
                        discarded.Add(record);
                }

                if (discarded.Count == 0)
                    return;

                InternalRewriteRecordsToDisk(survivors);

                _indexes.BeginUpdate();
                try
                {
                    discarded.ForEach(InternalRemoveIndex);
                }
                finally
                {
                    _indexes.EndUpdate();
                }
            }
        }

        #endregion ICompactableTable<T>

        #region Sequences

        public long NextSequence()
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(SimpleDBOperations<T>));

            return NextSequence(Consts.DefaultSequenceIncrement);
        }

        public long NextSequence(long increment)
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(SimpleDBOperations<T>));

            return InternalNextSequence(increment);
        }

        public long NextSecondarySequence(long increment)
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(SimpleDBOperations<T>));

            return InternalNextSecondarySequence(increment);
        }

        public void ResetSequence(long primarySequence, long secondarySequence)
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(SimpleDBOperations<T>));

            using (TimedLock timedLock = TimedLock.Lock(_lockObject))
            {
                using BinaryWriter writer = new(_fileStream, Encoding.UTF8, true);
                writer.Seek(Consts.PrimarySequenceStart, SeekOrigin.Begin);
                writer.Write(primarySequence);
                writer.Write(secondarySequence);

                _fileStream.Flush(true);

                _primarySequence = primarySequence;
                _SecondarySequence = secondarySequence;
            }
        }

        #endregion Sequences

        #endregion ISimpleDBOperation<T>

        #region ITransactionalSimpleDBOperations<T>

        public void Insert(T record, InsertOptions insertOptions, ITransaction transaction)
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(SimpleDBOperations<T>));

            if (record == null)
                throw new ArgumentNullException(nameof(record));

            if (transaction.AccessMode == TransactionAccessMode.ReadOnly)
                throw new TransactionException("Cannot update records in a read only transaction");

            using (StopWatchTimer timer = StopWatchTimer.Initialise(_ReadWriteTimes[TimingsInsert]))
            {
                InternalInsertRecords([record], insertOptions ?? new InsertOptions(), transaction);
            }
        }

        public void Insert(List<T> records, InsertOptions insertOptions, ITransaction transaction)
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(SimpleDBOperations<T>));

            if (records == null)
                throw new ArgumentNullException(nameof(records));

            if (records.Count == 0)
                throw new ArgumentException("Does not contain any records", nameof(records));

            if (transaction.AccessMode == TransactionAccessMode.ReadOnly)
                throw new TransactionException("Cannot insert records in a read only transaction");

            using (StopWatchTimer timer = StopWatchTimer.Initialise(_ReadWriteTimes[TimingsInsertList]))
            {
                InternalInsertRecords(records, insertOptions ?? new InsertOptions(), transaction);
            }
        }

        public void Update(T record, ITransaction transaction)
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(SimpleDBOperations<T>));

            if (record == null)
                throw new ArgumentNullException(nameof(record));

            if (transaction.AccessMode == TransactionAccessMode.ReadOnly)
                throw new TransactionException("Cannot update records in a read only transaction");

            using (StopWatchTimer timer = StopWatchTimer.Initialise(_ReadWriteTimes[TimingsUpdate]))
            {
                InternalUpdateRecords([record], transaction);
            }
        }

        public void Update(List<T> records, ITransaction transaction)
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(SimpleDBOperations<T>));

            if (records == null)
                throw new ArgumentNullException(nameof(records));

            if (transaction.AccessMode == TransactionAccessMode.ReadOnly)
                throw new TransactionException("Cannot update records in a read only transaction");

            using (StopWatchTimer timer = StopWatchTimer.Initialise(_ReadWriteTimes[TimingsUpdateList]))
            {
                InternalUpdateRecords(records, transaction);
            }
        }

        public void Delete(T record, ITransaction transaction)
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(SimpleDBOperations<T>));

            if (record == null)
                throw new ArgumentNullException(nameof(record));

            if (transaction.AccessMode == TransactionAccessMode.ReadOnly)
                throw new TransactionException("Cannot delete records in a read only transaction");

            using (StopWatchTimer timer = StopWatchTimer.Initialise(_ReadWriteTimes[TimingsDelete]))
            {
                InternalDeleteRecords([record], transaction);
            }
        }

        public void Delete(List<T> records, ITransaction transaction)
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(SimpleDBOperations<T>));

            if (records == null)
                throw new ArgumentNullException(nameof(records));

            if (transaction.AccessMode == TransactionAccessMode.ReadOnly)
                throw new TransactionException("Cannot delete records in a read only transaction");

            using (StopWatchTimer timer = StopWatchTimer.Initialise(_ReadWriteTimes[TimingsDeleteList]))
            {
                InternalDeleteRecords(records, transaction);
            }
        }

        public void Truncate(ITransaction transaction)
        {
            if (transaction.AccessMode == TransactionAccessMode.ReadOnly)
                throw new TransactionException("Cannot truncate records in a read only transaction");

            using (StopWatchTimer timer = StopWatchTimer.Initialise(_ReadWriteTimes[TimingsTruncate]))
            {
                InternalDeleteRecords(InternalReadAllRecords(), transaction);
            }
        }

        public void InsertOrUpdate(T record, ITransaction transaction)
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(SimpleDBOperations<T>));

            if (record == null)
                throw new ArgumentNullException(nameof(record));

            if (transaction.AccessMode == TransactionAccessMode.ReadOnly)
                throw new TransactionException("Cannot insert or update records in a read only transaction");

            using (StopWatchTimer timer = StopWatchTimer.Initialise(_ReadWriteTimes[TimingsInsertOrUpdate]))
            {
                if (IdExists(record.Id))
                    InternalUpdateRecords([record], transaction);
                else
                    InternalInsertRecords([record], new InsertOptions(), transaction);
            }
        }

        #endregion ITransactionalSimpleDBOperations<T>

        #region IWalUndoTarget

        public void UndoInsert(long recordId)
        {
            using (TimedLock timedLock = TimedLock.Lock(_lockObject))
            {
                List<T> existingRecords = InternalReadAllRecords();

                for (int i = existingRecords.Count - 1; i >= 0; i--)
                {
                    if (existingRecords[i].Id.Equals(recordId))
                    {
                        InternalRemoveIndex(existingRecords[i]);
                        existingRecords.RemoveAt(i);
                        break;
                    }
                }

                InternalApplyUndo(existingRecords);
            }
        }

        public void UndoUpdate(long recordId, byte[] undoRecord)
        {
            T restoredRecord = DeserializeRecord(undoRecord);

            using (TimedLock timedLock = TimedLock.Lock(_lockObject))
            {
                List<T> existingRecords = InternalReadAllRecords();

                for (int i = existingRecords.Count - 1; i >= 0; i--)
                {
                    if (existingRecords[i].Id.Equals(recordId))
                    {
                        InternalRemoveIndex(existingRecords[i]);
                        InternalAddIndex(restoredRecord);
                        existingRecords[i] = restoredRecord;
                        break;
                    }
                }

                InternalApplyUndo(existingRecords);
            }
        }

        public void UndoDelete(byte[] undoRecord)
        {
            T restoredRecord = DeserializeRecord(undoRecord);

            using (TimedLock timedLock = TimedLock.Lock(_lockObject))
            {
                List<T> existingRecords = InternalReadAllRecords();

                if (!existingRecords.Any(r => r.Id.Equals(restoredRecord.Id)))
                {
                    InternalAddIndex(restoredRecord);
                    existingRecords.Add(restoredRecord);
                }

                InternalApplyUndo(existingRecords);
            }
        }

        #endregion IWalUndoTarget

        #region Disposable

        public void Dispose()
        {
            Dispose(true);
            GC.SuppressFinalize(this);
        }

        #endregion Disposable

        #region Private Methods

        private long InternalNextSequence(long increment)
        {
            using (TimedLock timedLock = TimedLock.Lock(_lockObject))
            {
                _primarySequence += increment;

                using BinaryWriter writer = new(_fileStream, Encoding.UTF8, true);
                writer.Seek(Consts.PrimarySequenceStart, SeekOrigin.Begin);
                writer.Write(_primarySequence);

                if (_tableAttributes.WriteStrategy == WriteStrategy.Forced)
                    _fileStream.Flush(true);

                return _primarySequence;
            }
        }

        private long InternalNextSecondarySequence(long increment)
        {

            using (TimedLock timedLock = TimedLock.Lock(_lockObject))
            {
                _SecondarySequence += increment;

                using BinaryWriter writer = new(_fileStream, Encoding.UTF8, true);
                writer.Seek(Consts.SecondarySequenceStart, SeekOrigin.Begin);
                writer.Write(_SecondarySequence);

                if (_tableAttributes.WriteStrategy == WriteStrategy.Forced)
                    _fileStream.Flush(true);

                return _SecondarySequence;
            }
        }

        private List<T> InternalReadAllRecords()
        {
            OnAction?.Invoke(this);

                if (_allRecords != null)
                {
                    return _allRecords;
                }

            using (TimedLock timedLock = TimedLock.Lock(_lockObject))
            {
                IDataReader dataReader = _readWriteFactory.GetReader(_internalWriteVersion);

                int declaredRecordCount = InternalReadHeaderRecordCount();

                List<T> Result = dataReader.ReadRecords<T>(_fileStream, ref _pageCount, ref _recordCount, ref _dataLength);
                RefreshPhysicalDataLength();

                Result.ForEach(r => { r.Immutable = true; r.Loaded = true; });

                // A reader returning fewer records than the header advertises means the file was
                // torn by a crash part way through a rewrite. Repair it now, while we still hold
                // the lock and know exactly which records survived, because the append only writer
                // reads the record count back out of the header to decide where to append. Leaving
                // the stale count in place would make the next write skip past the gap and append
                // at an offset that no longer matches the data.
                if (declaredRecordCount > Result.Count)
                    InternalRepairTornRecords(Result);

                if (_isMemoryCaching)
                {
                    _allRecords = Result;
                    Volatile.Write(ref _inMemoryCacheSizeBytes, ComputeInMemoryCacheSize(Result));
                }

                return Result;
            }
        }

        private int InternalReadHeaderRecordCount()
        {
            if (_fileStream == null)
                return 0;

            using BinaryReader reader = new(_fileStream, Encoding.UTF8, true);
            _fileStream.Seek(Consts.StartOfRecordCount, SeekOrigin.Begin);
            _ = reader.ReadByte();
            return reader.ReadInt32();
        }

        /// <summary>
        /// Reads the physical/stored data length from the table header. Called after every read
        /// and write of the record set so that PhysicalDataSizeBytes can be answered without any
        /// file I/O.
        /// </summary>
        private void RefreshPhysicalDataLength()
        {
            using BinaryReader reader = new(_fileStream, Encoding.UTF8, true);
            _fileStream.Seek(Consts.StartOfRecordCount, SeekOrigin.Begin);
            _ = reader.ReadByte();
            _ = reader.ReadInt32();
            Volatile.Write(ref _physicalDataLength, reader.ReadInt32());
        }

        /// <summary>
        /// Computes the total serialized (UTF-8 JSON) byte size of the supplied records, used to
        /// keep InMemoryCacheSizeBytes current whenever the in-memory record set changes.
        /// </summary>
        private static long ComputeInMemoryCacheSize(List<T> records)
        {
            if (records == null || records.Count == 0)
                return 0;

            try
            {
                return JsonSerializer.SerializeToUtf8Bytes(records, records.GetType(), Consts.JsonSerializerOptions).LongLength;
            }
            catch
            {
                return 0;
            }
        }

        /// <summary>
        /// Rewrites the file so the header and the body agree again after a torn write has been
        /// detected on load.
        /// </summary>
        /// <remarks>
        /// Only writers that can shrink a file can perform this repair. The versioned writers
        /// rewrite the whole file on every save and so cannot produce a torn tail in the first
        /// place, meaning the absence of a compacting writer here is not an error - there is simply
        /// nothing to fix.
        /// </remarks>
        private void InternalRepairTornRecords(List<T> survivingRecords)
        {
            if (_readWriteFactory.GetWriter() is not ICompactingDataWriter compactingWriter)
                return;

            compactingWriter.RewriteData<T>(_fileStream, survivingRecords, _tableAttributes.Compression,
                _pageSize, ref _compactPercent, ref _pageCount);

            _recordCount = survivingRecords.Count;
        }

        private void InternalUpdateRecords(List<T> records, ITransaction transaction)
        {
            using (TimedLock timedLock = TimedLock.Lock(_lockObject))
            {
                records = records.Where(r => r.HasChanged).ToList();

                if (_foreignKeys.Count > 0)
                    ValidateForeignKeys(records);

                _indexes.BeginUpdate();
                try
                {
                    ITriggerContext triggerContext = new TriggerContext(transaction);
                    List<T> existingRecords = InternalReadAllRecords();

                    _triggersMap[TriggerType.BeforeUpdate].ForEach(t => t.BeforeUpdate(records, triggerContext));

                    foreach (T record in records)
                    {
                        for (int i = existingRecords.Count - 1; i >= 0; i--)
                        {
                            T existingRecord = existingRecords[i];
                            if (existingRecord.Id.Equals(record.Id))
                            {
                                _triggersMap[TriggerType.BeforeUpdateCompare].ForEach(t => t.BeforeUpdate(record, existingRecord, triggerContext));
                                long expectedUpdatedTicks = existingRecord.UpdatedTicks;
                                byte[] undoRecord = SerializeRecord(existingRecord);
                                InternalRemoveIndex(existingRecords[i]);
                                InternalAddIndex(record);
                                existingRecords[i] = record;
                                _walRecorder.Record(transaction, TableName, OperationType.Update, record.Id, SerializeRecord(record), expectedUpdatedTicks, undoRecord);
                                break;
                            }
                        }
                    }

                    InternalSaveRecordsToDisk(existingRecords, false, transaction);

                    _triggersMap[TriggerType.AfterUpdate].ForEach(t => t.AfterUpdate(records, triggerContext));

                    records.ForEach(r => r.HasChanged = false);
                }
                finally
                {
                    _indexes.EndUpdate();
                }
            }
        }

        /// <summary>
        /// Flushes the in-memory record set to the table file. Deliberately does not open a
        /// transaction - a checkpoint generates no WAL entries, and taking a transaction here would
        /// couple flushing (including the final flush performed during Dispose) to the WAL table,
        /// which may already have been disposed by the container, silently losing the checkpoint.
        /// </summary>
        private void InternalCheckpoint()
        {
            if (_tableAttributes == null || _tableAttributes.WriteStrategy == WriteStrategy.Forced)
                return;

            using (TimedLock timedLock = TimedLock.Lock(_lockObject, SimpleDBConfiguration.ClearMemoryLockTimeout))
            {
                InternalSaveRecordsToDisk(InternalReadAllRecords(), true, null);
            }
        }

        private void InternalSaveRecordsToDisk(List<T> recordsToSave, bool forceWrite, ITransaction transaction)
        {
            _recordCount = recordsToSave.Count;

            // Lazy tables (the default) deliberately do not touch disk here - the WAL is the
            // durability record between checkpoints and is force written at commit. The table
            // file catches up when a checkpoint runs, and PerformStartupRecovery replays any
            // committed WAL entries beyond the watermark if we crash before that happens.
            if (forceWrite || _tableAttributes.WriteStrategy == WriteStrategy.Forced)
            {
                IDataWriter dataWriter = _readWriteFactory.GetWriter();
                dataWriter.WriteData<T>(_fileStream, recordsToSave, _tableAttributes.Compression,
                    _pageSize, ref _compactPercent, ref _pageCount);

                if (_internalWriteVersion < dataWriter.Version)
                {
                    using BinaryWriter writer = new(_fileStream, Encoding.UTF8, true);
                    _fileStream.Seek(Consts.WriteVersionStart, SeekOrigin.Begin);
                    writer.Write(dataWriter.Version);
                    _internalWriteVersion = dataWriter.Version;
                }

                _fileStream.Flush(true);
                RefreshPhysicalDataLength();
            }



            if (_isMemoryCaching)
            {
                _allRecords = recordsToSave;
                _allRecords.ForEach(ar => { ar.Immutable = true; ar.Loaded = true; });
                Volatile.Write(ref _inMemoryCacheSizeBytes, ComputeInMemoryCacheSize(_allRecords));
            }
            else
            {
                _allRecords = null;
                Volatile.Write(ref _inMemoryCacheSizeBytes, 0);
            }

            OnAction?.Invoke(this);
        }

        /// <summary>
        /// Rewrites the table file from scratch so that records removed from the set actually
        /// release their disk space. The versioned writers already rewrite the whole file on every
        /// save, but the append only writer can only add to the tail, so it needs an explicit
        /// rewrite - without it the header record count would shrink while the obsolete bytes
        /// stayed on disk and the reader would return the oldest records rather than the survivors.
        /// </summary>
        private void InternalRewriteRecordsToDisk(List<T> recordsToSave)
        {
            _recordCount = recordsToSave.Count;

            IDataWriter dataWriter = _readWriteFactory.GetWriter();

            if (dataWriter is ICompactingDataWriter compactingWriter)
            {
                compactingWriter.RewriteData<T>(_fileStream, recordsToSave, _tableAttributes.Compression,
                    _pageSize, ref _compactPercent, ref _pageCount);
            }
            else
            {
                dataWriter.WriteData<T>(_fileStream, recordsToSave, _tableAttributes.Compression,
                    _pageSize, ref _compactPercent, ref _pageCount);
            }

            if (_internalWriteVersion < dataWriter.Version)
            {
                using BinaryWriter writer = new(_fileStream, Encoding.UTF8, true);
                _fileStream.Seek(Consts.WriteVersionStart, SeekOrigin.Begin);
                writer.Write(dataWriter.Version);
                _internalWriteVersion = dataWriter.Version;
            }

            _fileStream.Flush(true);
            RefreshPhysicalDataLength();

            if (_isMemoryCaching)
            {
                _allRecords = recordsToSave;
                _allRecords.ForEach(ar => { ar.Immutable = true; ar.Loaded = true; });
                Volatile.Write(ref _inMemoryCacheSizeBytes, ComputeInMemoryCacheSize(_allRecords));
            }
            else
            {
                _allRecords = null;
                Volatile.Write(ref _inMemoryCacheSizeBytes, 0);
            }

            OnAction?.Invoke(this);
        }

        private void InternalDeleteRecords(List<T> records, ITransaction transaction)
        {
            using (TimedLock timedLock = TimedLock.Lock(_lockObject))
            {
                ValidateForeignKeysPriorToDelete(records);

                _indexes.BeginUpdate();
                try
                {
                    ITriggerContext triggerContext = new TriggerContext(transaction);

                    _triggersMap[TriggerType.BeforeDelete].ForEach(t => t.BeforeDelete(records, triggerContext));

                    List<T> existingRecords = InternalReadAllRecords();

                    for (int i = records.Count - 1; i >= 0; i--)
                    {
                        T record = records[i];

                        for (int j = existingRecords.Count - 1; j >= 0; j--)
                        {
                            T existingRecord = existingRecords[j];

                            if (existingRecord.Id.Equals(record.Id))
                            {
                                existingRecords.RemoveAt(j);
                                InternalRemoveIndex(existingRecord);
                                byte[] serializedExisting = SerializeRecord(existingRecord);
                                _walRecorder.Record(transaction, TableName, OperationType.Delete, existingRecord.Id, serializedExisting, existingRecord.UpdatedTicks, serializedExisting);

                                break;
                            }

                        }
                    }

                    InternalSaveRecordsToDisk(existingRecords, false, transaction);

                    _triggersMap[TriggerType.AfterDelete].ForEach(t => t.AfterDelete(records, triggerContext));
                }
                finally
                {
                    _indexes.EndUpdate();
                }
            }
        }

        private void InternalInsertRecords(List<T> records, InsertOptions textTableInsertOptions, ITransaction transaction)
        {
            using (TimedLock timedLock = TimedLock.Lock(_lockObject))
            {
                ValidateInternalIndexes(records);

                long nextSequence;

                if (textTableInsertOptions.AssignPrimaryKey)
                {
                    nextSequence = PrimarySequence + 1;
                    _ = NextSequence(records.Count);
                }
                else
                {
                    nextSequence = 0;
                }

                _indexes.BeginUpdate();
                try
                {
                    ITriggerContext triggerContext = new TriggerContext(transaction);

                    _triggersMap[TriggerType.BeforeInsert].ForEach(t => t.BeforeInsert(records, triggerContext));

                    records.ForEach(r =>
                    {
                        ValidateForeignKeys(r);
                        if (textTableInsertOptions.AssignPrimaryKey)
                        {
                            r.Id = nextSequence++;
                        }

                        InternalAddIndex(r);
                        _walRecorder.Record(transaction, TableName, OperationType.Insert, r.Id, SerializeRecord(r), -1L, null);
                    });

                    List<T> existingRecords = InternalReadAllRecords();
                    existingRecords.AddRange(records);
                    InternalSaveRecordsToDisk(existingRecords, false, transaction);

                    _triggersMap[TriggerType.AfterInsert].ForEach(t => t.AfterInsert(records, triggerContext));
                }
                finally
                {
                    _indexes.EndUpdate();
                }
            }
        }

        private void ValidateInternalIndexes(List<T> records)
        {
            foreach (KeyValuePair<string, IIndexManager> item in _indexes)
            {
                foreach (T record in records)
                {
                    object keyValue = GetIndexValue(record, item.Value);

                    if (_indexes[item.Key].Contains(keyValue))
                    {
                        throw new UniqueIndexException($"Index already exists; Table: {TableName}; Index Name: {item.Key}; Property: {String.Join(',', item.Value.PropertyNames)}; Value: {keyValue}");
                    }
                }
            }
        }

        private void InternalAddIndex(T record)
        {
            foreach (KeyValuePair<string, IIndexManager> item in _indexes)
            {
                object keyValue = GetIndexValue(record, item.Value);

                _indexes[item.Key].Add(keyValue);
            }
        }

        private void InternalRemoveIndex(T record)
        {
            foreach (KeyValuePair<string, IIndexManager> item in _indexes)
            {
                object value = GetIndexValue(record, item.Value);

                _indexes[item.Key].Remove(value);
            }
        }

        private void RebuildAllMissingIndexes()
        {
            IReadOnlyList<T> allRecords = Select();

            foreach (KeyValuePair<string, IIndexManager> item in _indexes)
            {
                item.Value.BeginUpdate();
                try
                {
                    foreach (T record in allRecords)
                    {
                        object keyValue = GetIndexValue(record, item.Value);

                        if (!_indexes[item.Key].Contains(keyValue))
                            _indexes[item.Key].Add(keyValue);
                    }
                }
                finally
                {
                    item.Value.EndUpdate();
                }
            }
        }

        private void ValidateForeignKeysPriorToDelete(List<T> records)
        {
            foreach (KeyValuePair<string, IIndexManager> index in _indexes)
            {
                if (index.Value.PropertyNames.Count > 1)
                    continue;

                foreach (T record in records)
                {
                    object keyValue = record.GetType().GetProperty(index.Key).GetValue(record, null);

                    if (Int64.TryParse(keyValue.ToString(), out long value))
                    {
                        ForeignKeyUsage foreignKeyUsage = _foreignKeyManager.ValueInUse(TableName, index.Key, value, out string table, out string propertyName);

                        if (foreignKeyUsage == ForeignKeyUsage.Referenced &&
                            foreignKeyUsage != ForeignKeyUsage.AllowDefault && foreignKeyUsage != ForeignKeyUsage.CascadeDelete)
                        {
                            throw new ForeignKeyException($"Foreign key value {keyValue} from table {TableName} is being used in Table: {table}; Property: {propertyName}");
                        }
                    }
                }
            }
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void ValidateForeignKeys(List<T> records)
        {
            foreach (KeyValuePair<string, ForeignKeyRelation> foreignKey in _foreignKeys)
            {
                foreach (T record in records)
                {
                    long keyValue = Convert.ToInt64(record.GetType().GetProperty(foreignKey.Key).GetValue(record, null));
                    ForeignKeyRelation foreignKeyRelation = foreignKey.Value;

                    if (!_foreignKeyManager.ValueExists(foreignKeyRelation.Name, keyValue) && !(foreignKeyRelation.Attributes == ForeignKeyAttributes.DefaultValue && keyValue.Equals(0)))
                        throw new ForeignKeyException($"Foreign key value {keyValue} does not exist in table {foreignKey.Value}; Table: {TableName}; Property: {foreignKey.Key}");
                }
            }
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void ValidateForeignKeys(T record)
        {
            foreach (KeyValuePair<string, ForeignKeyRelation> foreignKey in _foreignKeys)
            {
                long keyValue = Convert.ToInt64(record.GetType().GetProperty(foreignKey.Key).GetValue(record, null));
                ForeignKeyRelation foreignKeyRelation = foreignKey.Value;

                if (!_foreignKeyManager.ValueExists(foreignKeyRelation.Name, keyValue) && !(foreignKeyRelation.Attributes == ForeignKeyAttributes.DefaultValue && keyValue.Equals(0)))
                    throw new ForeignKeyException($"Foreign key value {keyValue} does not exist in table {foreignKey.Value}; Table: {TableName}; Property: {foreignKey.Key}");
            }
        }

        private Dictionary<string, ForeignKeyRelation> GetForeignKeysForTable()
        {
            Dictionary<string, ForeignKeyRelation> Result = [];

            foreach (PropertyInfo property in typeof(T).GetProperties())
            {
                ForeignKeyAttribute foreignKey = (ForeignKeyAttribute)property.GetCustomAttributes(true)
                    .FirstOrDefault(ca => ca.GetType().Equals(typeof(ForeignKeyAttribute)));

                if (foreignKey != null && property.PropertyType.Equals(typeof(long)))
                {
                    _foreignKeyManager.AddRelationShip(TableName, foreignKey.TableName, property.Name, foreignKey.PropertyName, foreignKey.Attributes);
                    Result.Add(property.Name, new ForeignKeyRelation(foreignKey.TableName, foreignKey.Attributes));
                }
            }

            return Result;
        }

        private static TableAttribute GetTableAttributes()
        {
            return (TableAttribute)typeof(T).GetCustomAttributes(true)
                .FirstOrDefault(a => a.GetType() == typeof(TableAttribute));
        }

        private static SimpleDB.Attributes.SystemTableAttribute GetSystemTableAttribute()
        {
            return (SimpleDB.Attributes.SystemTableAttribute)typeof(T).GetCustomAttributes(true)
                .FirstOrDefault(a => a.GetType() == typeof(SimpleDB.Attributes.SystemTableAttribute));
        }

        private static IVersionedReadWriteFactory ResolveReadWriteFactory()
        {
            SimpleDB.Attributes.SystemTableAttribute systemTableAttribute = GetSystemTableAttribute();

            if (systemTableAttribute != null && systemTableAttribute.StorageEngine == StorageEngine.AppendOnly)
                return new AppendOnlyReadWriteFactory();

            return new VersionedReadWriteFactory();
        }

        private IWalRecorder ResolveWalRecorder()
        {
            return TableParticipatesInWal() ? new WalRecorder(_walTableAccessor) : new NullWalRecorder();
        }

        /// <summary>
        /// Ordinary user tables (no [SystemTable]) always participate in WAL. System tables only
        /// participate if explicitly opted in via ParticipatesInWal, e.g. a future audit/config
        /// system table - never the WAL table itself.
        /// </summary>
        private static bool TableParticipatesInWal()
        {
            SimpleDB.Attributes.SystemTableAttribute systemTableAttribute = GetSystemTableAttribute();

            return systemTableAttribute == null || systemTableAttribute.ParticipatesInWal;
        }

        private static byte[] SerializeRecord(T record)
        {
            return JsonSerializer.SerializeToUtf8Bytes(record, Consts.JsonSerializerOptions);
        }

        private static T DeserializeRecord(byte[] serializedRecord)
        {
            if (serializedRecord == null)
                throw new ArgumentNullException(nameof(serializedRecord));

            return JsonSerializer.Deserialize<T>(serializedRecord, Consts.JsonSerializerOptions);
        }

        /// <summary>
        /// Persists the record set produced by an undo operation. Deliberately does not record
        /// WAL entries or fire triggers - the state being restored was previously produced by
        /// this table and the transaction that produced it is being discarded.
        /// </summary>
        private void InternalApplyUndo(List<T> recordsToRestore)
        {
            InternalSaveRecordsToDisk(recordsToRestore, false, null);
        }

        /// <summary>
        /// Replays any committed WAL entries for this table that have not yet been reflected in
        /// the on-disk table file, then performs a single checkpoint flush and watermark update.
        /// Skipped for system tables (e.g. the WAL table and the watermark table itself) to avoid
        /// recursively resolving those tables while they are still being constructed.
        /// </summary>
        private void PerformStartupRecovery()
        {
            if (GetSystemTableAttribute() != null)
                return;

            using (StopWatchTimer timer = StopWatchTimer.Initialise(_ReadWriteTimes[TimingsRecovery]))
            {
                // Replay holds the database lock so no other thread can read or mutate tables while
                // the WAL is being rolled forward. The lock is reentrant, so writing the checkpoint
                // watermark below - which begins its own transaction - reacquires it harmlessly.
                IDisposable scope = (_simleDBManager as IDatabaseLock)?.AcquireForOperation();

                try
                {
                    InternalPerformStartupRecovery();
                }
                finally
                {
                    scope?.Dispose();
                }
            }
        }

        private void InternalPerformStartupRecovery()
        {
            long lastFlushedSequence = _watermarkAccessor.GetLastFlushedSequence(TableName);

            // Safe against a truncated WAL: truncation only ever discards entries at or below the
            // MINIMUM watermark across all participating tables, which by definition is at or below
            // this table's own watermark - so every discarded entry would have been filtered out
            // here anyway. A table seeded at -1 has discarded nothing and replays its full log.
            List<WalEntryDataRow> entriesToReplay = _walTableAccessor.WalTable
                .Select(w => w.TableName == TableName && w.SequenceNumber > lastFlushedSequence)
                .OrderBy(w => w.SequenceNumber)
                .ToList();

            if (entriesToReplay.Count == 0)
                return;

            // Entries are logged as each operation happens, not at commit, so the log contains
            // work belonging to transactions that never committed. Only those with a Commit
            // marker may be replayed; everything else is a loser and must be reversed instead.
            HashSet<long> committedTransactions = _walTableAccessor.WalTable
                .Select(w => w.Operation == OperationType.Commit)
                .Select(w => w.TransactionId)
                .ToHashSet();

            List<T> workingSet = new(InternalReadAllRecords());
            long highestSequence = lastFlushedSequence;

            // Redo pass. Replays committed work in log order, bringing the table file up to date.
            foreach (WalEntryDataRow entry in entriesToReplay)
            {
                highestSequence = Math.Max(highestSequence, entry.SequenceNumber);

                if (!committedTransactions.Contains(entry.TransactionId))
                    continue;

                workingSet.RemoveAll(r => r.Id.Equals(entry.RecordId));

                if (entry.Operation != OperationType.Delete)
                    workingSet.Add(DeserializeRecord(entry.SerializedRecord));
            }

            // Undo pass. A loser is a transaction that was rolled back (Abort marker) or was
            // interrupted before it could commit at all. Either way its operations were already
            // applied to the record set, and a WriteStrategy.Forced table will have flushed them,
            // so they must be reversed here - the in-memory rollback that would normally have
            // done it did not survive the crash. Reverse order, so repeated operations against
            // the same record unwind correctly.
            for (int i = entriesToReplay.Count - 1; i >= 0; i--)
            {
                WalEntryDataRow entry = entriesToReplay[i];

                if (committedTransactions.Contains(entry.TransactionId))
                    continue;

                workingSet.RemoveAll(r => r.Id.Equals(entry.RecordId));

                // Insert has no before-image - reversing it simply leaves the record removed.
                if (entry.Operation != OperationType.Insert && entry.UndoRecord != null)
                    workingSet.Add(DeserializeRecord(entry.UndoRecord));
            }

            InternalSaveRecordsToDisk(workingSet, true, null);
            _watermarkAccessor.SetLastFlushedSequence(TableName, highestSequence);
        }

        private static BatchUpdateDictionary<string, IIndexManager> BuildIndexListForTable()
        {
            BatchUpdateDictionary<string, IIndexManager> Result = [];
            foreach (PropertyInfo property in typeof(T).GetProperties())
            {
                UniqueIndexAttribute uniqueIndex = (UniqueIndexAttribute)property.GetCustomAttributes(true)
                    .FirstOrDefault(ca => ca.GetType().Equals(typeof(UniqueIndexAttribute)));

                if (uniqueIndex != null)
                {
                    string indexName = String.IsNullOrEmpty(uniqueIndex.Name) ? property.Name : uniqueIndex.Name;

                    if (Result.TryGetValue(indexName, out IIndexManager value))
                    {
                        List<string> propertyNames = value.PropertyNames;
                        propertyNames.Add(property.Name);
                        Result.Remove(indexName);
                        Result.Add(indexName, new IndexManager<string>(uniqueIndex.IndexType, [.. propertyNames]));
                    }
                    else
                    {
                        if (property.PropertyType == typeof(long))
                            Result.Add(indexName, new IndexManager<long>(uniqueIndex.IndexType, property.Name));
                        else if (property.PropertyType == typeof(string))
                            Result.Add(indexName, new IndexManager<string>(uniqueIndex.IndexType, property.Name));
                        else if (property.PropertyType == typeof(int))
                            Result.Add(indexName, new IndexManager<int>(uniqueIndex.IndexType, property.Name));
                        else if (property.PropertyType == typeof(float))
                            Result.Add(indexName, new IndexManager<float>(uniqueIndex.IndexType, property.Name));
                        else if (property.PropertyType == typeof(double))
                            Result.Add(indexName, new IndexManager<double>(uniqueIndex.IndexType, property.Name));
                        else if (property.PropertyType == typeof(decimal))
                            Result.Add(indexName, new IndexManager<decimal>(uniqueIndex.IndexType, property.Name));
                        else if (property.PropertyType == typeof(uint))
                            Result.Add(indexName, new IndexManager<uint>(uniqueIndex.IndexType, property.Name));
                        else if (property.PropertyType == typeof(ulong))
                            Result.Add(indexName, new IndexManager<ulong>(uniqueIndex.IndexType, property.Name));
                        else if (property.PropertyType == typeof(short))
                            Result.Add(indexName, new IndexManager<short>(uniqueIndex.IndexType, property.Name));
                        else if (property.PropertyType == typeof(ushort))
                            Result.Add(indexName, new IndexManager<ushort>(uniqueIndex.IndexType, property.Name));
                        else if (property.PropertyType == typeof(bool))
                            Result.Add(indexName, new IndexManager<bool>(uniqueIndex.IndexType, property.Name));
                        else
                            throw new InvalidOperationException($"Type {property.PropertyType.Name} not supported");
                    }
                }
            }

            return Result;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static object GetIndexValue(T record, IIndexManager indexManager)
        {
            if (indexManager.PropertyNames.Count == 1)
            {
                return record.GetType().GetProperty(indexManager.PropertyNames[0]).GetValue(record, null);
            }
            else
            {
                StringBuilder sb = new();

                foreach (string propertyName in indexManager.PropertyNames)
                {
                    sb.Append(record.GetType().GetProperty(propertyName).GetValue(record, null) ?? String.Empty);
                }

                return sb.ToString();
            }
        }

        private void Dispose(bool disposing)
        {
            if (_disposed)
                return;

            if (disposing)
            {
                _simleDBManager?.UnregisterTable(this);

                if (_foreignKeyManager != null)
                    _foreignKeyManager.UnregisterTable(this);
            }

            // Checkpointing performs file I/O (and, transitively, may touch other managed objects
            // such as the read/write factory) and is therefore unsafe to run from the finalizer
            // thread: the underlying file stream, or objects it depends on, may already have been
            // finalized/disposed by the time this runs, and order of finalization is not
            // guaranteed. Skip it on the finalizer path - any unflushed data is recoverable from
            // the WAL on next startup - and only attempt it when explicitly disposing, and even
            // then only if the file stream is actually available.
            if (disposing && _fileStream != null)
                InternalCheckpoint();

            // The shutdown flush is only useful for recovery if the watermark records it, otherwise
            // startup replays work already on disk. Truncation and compaction are deliberately NOT
            // performed here - they rewrite the WAL, the very file recovery depends on, at the point
            // where the container may already have disposed it.
            //
            // Only attempted when disposing: on the finalizer path the WAL and watermark tables may
            // already have been collected. A failure degrades to replaying on next startup, which is
            // correct but slower, so it must never prevent the file handle below being released.
            if (disposing && ParticipatesInWal)
            {
                try
                {
                    _watermarkAccessor?.SetLastFlushedSequence(TableName, _walTableAccessor.WalTable.SecondarySequence);
                }
                catch (ObjectDisposedException)
                {
                    // WAL or watermark table already disposed - recovery replays instead.
                }
                catch (InvalidOperationException)
                {
                    // Service provider already disposed - recovery replays instead.
                }
            }

            if (_fileStream != null)
            {
                _fileStream.Flush(true);
                _fileStream.Close();
                _fileStream.Dispose();
            }

            _disposed = true;
        }

        private void ValidateTableContents()
        {
            using BinaryReader reader = new(_fileStream, Encoding.UTF8, true);
            _fileStream.Seek(0, SeekOrigin.Begin);

            _internalDataVersion = reader.ReadUInt16();

            Span<byte> header = reader.ReadBytes(Consts.HeaderLength);

            for (int i = 0; i < header.Length; i++)
            {
                if (header[i] != Consts.Header[i])
                    throw new InvalidDataException();
            }

            _primarySequence = reader.ReadInt64();
            _SecondarySequence = reader.ReadInt64();
            _internalWriteVersion = reader.ReadUInt16();
            _ = reader.ReadUInt16();
            _ = reader.ReadInt32();
            _ = reader.ReadInt32();
            _pageSize = (PageSize)reader.ReadInt32();
            _ = (CompressionType)reader.ReadByte();
            _recordCount = reader.ReadInt32();
            _physicalDataLength = reader.ReadInt32();
            _dataLength = reader.ReadInt32();
        }

        private (bool, string) ValidateTableName(string path, string domain, string name, PageSize pageSize)
        {
            string extension = Path.GetExtension(name);

            if (String.IsNullOrEmpty(extension))
                name += Consts.DefaultExtension;

            string tableName = path;

            if (!String.IsNullOrEmpty(domain))
                tableName = Path.Combine(tableName, domain);

            if (!Directory.Exists(tableName))
                Directory.CreateDirectory(tableName);

            tableName = Path.Combine(tableName, name);

            bool tableCreated = false;

            if (!File.Exists(tableName))
            {
                CreateTableHeaderRecords(tableName, pageSize);
                tableCreated = true;
            }

            return (tableCreated, tableName);
        }

        private ushort InternalUpdateVersion(ushort version)
        {
            using (TimedLock timedLock = TimedLock.Lock(_lockObject))
            {
                using BinaryWriter writer = new(_fileStream, Encoding.UTF8, true);
                writer.Seek(Consts.DataVersionStart, SeekOrigin.Begin);
                writer.Write(version);
                _fileStream.Flush(true);
            }

            return version;
        }

        private void CreateTableHeaderRecords(string fileName, PageSize pageSize)
        {
            using FileStream stream = File.Open(fileName, FileMode.OpenOrCreate);
            using BinaryWriter writer = new(stream, Encoding.UTF8, false);
            writer.Seek(0, SeekOrigin.Begin);

            writer.Write(FileVersion);
            writer.Write(Consts.Header);
            writer.Write(_primarySequence);
            writer.Write(_SecondarySequence);
            writer.Write((ushort)0);
            writer.Write((ushort)0);
            writer.Write((int)0);
            writer.Write((int)0);
            writer.Write((int)pageSize);
            writer.Write((byte)_tableAttributes.Compression);
            writer.Write(Consts.RowCountZero);
            writer.Write(Consts.DefaultLength);
            writer.Write(Consts.DefaultLength);
            writer.Write(Consts.DefaultLength);

            writer.Flush();
        }

        #endregion Private Methods
    }
}
